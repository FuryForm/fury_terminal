...
/**
 * Client-side: read one DATA message, skipping non-DATA messages.
 * Returns bytes read, 0 on CLOSE, -1 on error.
 */
static inline int proto_recv_data(int fd, uint8_t *buf, int buf_size) {
    for (;;) {
        uint8_t hdr[5];
        if (proto_read_exact(fd, hdr, 5) < 0) return -1;
        uint8_t  type = hdr[0];
        ...
        if (len > FTYD_MAX_MSG_LEN) return -1; /* SEC-3: reject oversized */

        if (type == FTYD_DATA) {
            if ((int)len > buf_size) {
                /* Read what fits, drain excess (BUG-3 fix) */
                if (proto_read_exact(fd, buf, buf_size) < 0) return -1;
                if (proto_drain(fd, len - (uint32_t)buf_size) < 0) return -1;
                return buf_size;
            }
